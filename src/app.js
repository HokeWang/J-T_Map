...
function initializeMap() {
  ...
  createSchemeMaps();
  setStatus("地图已加载，可以开始浏览器直连解析。部分供应商可能受 CORS 限制。");
  updateRunState();
}
...
function isGoogleProvider(provider) {
  const value = `${provider.name || ""} ${provider.url || ""}`.toLowerCase();
  return value.includes("google") || value.includes("googleapis.com/maps/api/geocode");
}

function geocodeWithGoogleMaps(provider, scheme, address) {
  if (!state.geocoder) {
    return Promise.resolve({ provider: provider.name, scheme: scheme.name, schemeId: scheme.id, key: makeResultKey(provider.name, scheme.name), address, error: "MAP_NOT_READY", message: "请先加载 Google 地图。" });
  }
  ...
  return state.geocoder.geocode(request)
    .then(({ results }) => {
      const firstResult = Array.isArray(results) ? results[0] : null;
      const location = firstResult?.geometry?.location;
      if (!location) {
        return { provider: provider.name, scheme: scheme.name, schemeId: scheme.id, key: makeResultKey(provider.name, scheme.name), address, error: "ZERO_RESULTS", message: "Google 未返回可用坐标。" };
      }
      return {
        provider: provider.name,
        scheme: scheme.name,
        schemeId: scheme.id,
        key: makeResultKey(provider.name, scheme.name),
        address,
        lat: location.lat(),
        lng: location.lng(),
...
function geocodeWithDirectFetch(provider, scheme, address) {
  ...
  let request;
  try {
    request = buildDirectVendorRequest(provider, values);
  } catch (error) {
    return Promise.resolve({ provider: provider.name, scheme: scheme.name, schemeId: scheme.id, key: makeResultKey(provider.name, scheme.name), address, error: "INVALID_PROVIDER_CONFIG", message: error.message });
  }
  ...
  return fetch(request.url, request.options)
    .then(async (response) => {
      ...
      if (!response.ok || payload.error) {
        return { provider: provider.name, scheme: scheme.name, schemeId: scheme.id, key: makeResultKey(provider.name, scheme.name), address, error: payload.error?.code || payload.error || `HTTP_${response.status}`, message: extractVendorErrorMessage(payload) };
      }
...
function buildDirectVendorRequest(provider, values) {
  const targetUrl = String(provider.url || "").trim();
  if (!targetUrl || !provider.latPath || !provider.lngPath) {
    throw new Error("url、latPath、lngPath 不能为空。");
  }
  ...
  const method = String(provider.method || "GET").toUpperCase() === "POST" ? "POST" : "GET";
